...
# Ethimedis base URL with type_id=5 for Assistenzarzt filtering
ETHIMEDIS_BASE_URL = (
...
class EthimedisScraper:
    """Scraper for ethimedis.de via legacy AJAX endpoint (JSON with HTML content)."""

    source_name = "ethimedis"

    def parse_html_content(self, html: str) -> list[ScrapedJob]:
        """Parse the HTML content field from the Ethimedis JSON response."""
        ...
        for block in page.css("[data-jobofferid]"):
            ...
            # Company: text before the <h5> title in the block
            company = self._extract_company(block)

            # Location: <span class="look_text">
            location = ""
    ...
    async def scrape(self, max_pages: int = 50) -> tuple[list[ScrapedJob], int, list[str]]:
        """Scrape Ethimedis via AJAX endpoint with offset-based pagination."""
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            for page_num in range(max_pages):
                ...
                try:
                    ...
                    jobs = self.parse_html_content(html_content)
